...
async function ChatByVenice(q: string) {
  try {
    ...
    const veniceResponse = await axios.post(
      "https://venice.ai/api/inference/chat",
      {
        prompt: [
          {
            content:
              "you are the best search enginge answer accrodingly to my queires,Create a personalized search experience by using machine learning to tailor results based on individual user preferences and interests",
            role: "user",
          },
          {
            content: "ok sure!",
    ...
    if (veniceResponse.status !== 200) {
      throw new Error(
        `Venice API request failed with status ${veniceResponse.status}`
      );
    }
