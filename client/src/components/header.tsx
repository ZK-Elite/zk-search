...
const router = useRouter();
  ...
  // Check if the pathname is either /search or /news
  const isSearchOrNews = pathname === "/search" || pathname === "/news";

  return (
    <div className={`fixed px-5 w-full z-40 ${isSearchOrNews ? "backdrop-blur" : ""}`}>
      <div className="flex w-full md:px-4 pt-6 pb-4 items-end">
        {isSearchOrNews && (
          <div className="flex sm:flex-row flex-col w-full gap-3">
            <div className="flex items-center justify-start md:w-20 w-full">
              ...
              <div className={`ml-auto flex sm:hidden cursor-pointer bg-opacity-70`}>
                <ThemeButton />
              </div>
            </div>
        ...
        <div className={`ml-auto sm:flex hidden cursor-pointer bg-opacity-70 ${pathname === "/" && "mt-3.5"}`}>
          <ThemeButton />
        </div>
      </div>
      {isSearchOrNews && (
          ...
          {buttonTabs.map((item, index) => (
            ...
            >
              <Image
                ...
                priority
              />
              <span className="text-base hidden sm:flex font-medium">{item.title}</span>
            </button>
          ))}
        </div>
      )}
      <div className={`${isSearchOrNews && "mx-4 border-b border-[#27272A] sm:flex hidden"}`}></div>
    </div>
  );
};
